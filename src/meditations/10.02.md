...
The idea that is new to us in recovery is that we _can choose_. We can set the boundaries we need to set with family members. We can choose a path that works for us, without guilt and obligation or undue influence from any source, including recovery professionals.
...
It's okay to say no to our families when that is what we want. It's okay to say yes to our families if that feels right. It's okay to call timeout and it's okay to go back as a different person.
